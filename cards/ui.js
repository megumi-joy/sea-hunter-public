...
function renderIslandsList() {
  ...
  html += '<p class="rules-note">Frontline and reserve: attacks only ever target the enemy frontline. The reserve never fights and is never attacked -- on your turn a reserve card may instead move straight ahead into the empty frontline slot in its own column. The round ends the moment a side\'s frontline is empty, and that side loses the round; if one draw empties both frontlines at once, the round is drawn.</p>';
